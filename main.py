...
--- НАСТРОЙКИ ---
BOT_TOKEN = "8839261922:AAEwe-AUMXSmk0Fap2n1VpMQ_E8sSEg41s8" bot = Bot(token=BOT_TOKEN) dp = Dispatcher()
--- ЛОГИКА СКАЧИВАНИЯ С YOUTUBE ---
def download_youtube_video(url, output_path): """Скачивает видео с YouTube с помощью yt-dlp"""
...
with yt_dlp.YoutubeDL(ydl_opts) as ydl: 
  ydl.download([url]) return True except Exception as e: print(f"Ошибка yt-dlp: {e}") return False
--- ЛОГИКА ОБРАБОТКИ ВИДЕО (старая функция) ---
def extract_best_moment(input_path, output_path, audio_path, clip_before=15, clip_after=5): try: video = VideoFileClip(input_path) if video.audio is None: return False
...
@dp.message(Command("start")) async def cmd_start(message: types.Message): await message.answer("Привет! Отправь мне видео (до 20МБ) ИЛИ ссылку на YouTube, и я найду самый громкий момент!")
Хендлер для ссылок на YouTube
@dp.message(F.text.regexp(r"(youtube.com|youtu.be)")) async def handle_youtube_link(message: types.Message): url = message.text.strip() msg = await message.reply("🔗 Вижу ссылку на YouTube! Начинаю скачивание (это займет какое-то время)...")
...
@dp.message(F.video) async def handle_video(message: types.Message): if message.video.file_size > 20 * 1024 * 1024: await message.reply("Видео слишком большое! Отправь файл до 20 МБ или скинь ссылку на YouTube.") return # ... здесь код обработки обычного файла из предыдущего сообщения ... await message.reply("Функция обработки файлов работает в штатном режиме!")
async def main(): print("Бот запущен! Жду ссылки и файлы...") await dp.start_polling(bot)
